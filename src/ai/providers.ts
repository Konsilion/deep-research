...
function createModelWithReasoning(modelName: string): LanguageModelV1 {
  console.log(`Création du modèle ${modelName} avec middleware de raisonnement`);
...
function createSimpleModel(modelName: string): LanguageModelV1 {
  console.log(`Création du modèle ${modelName} sans middleware de raisonnement`);
...
export function getModel(): LanguageModelV1 {
  if (!modelInstance) {
    ...
    console.log(`Modèle principal initialisé: ${modelName} (avec raisonnement: ${useThinkingModels})`);
  }
...
export function getIntroductionModel(): LanguageModelV1 {
  if (!introductionModelInstance) {
    const modelName = process.env.CUSTOM_MODEL_INTRO || process.env.CUSTOM_MODEL || 'llama3';
    ...
    console.log(`Modèle d'introduction initialisé: ${modelName} (toujours sans raisonnement)`);
  }
...
export function trimPrompt(
  ...
  console.log(`Prompt trop long : ${length} tokens, réduction à ${adjustedContextSize} tokens (limite originale: ${contextSize}, marge: ${securityMargin})`);
  ...
  if (length > contextSize * 1.5) {
    ...
    const endChunk = prompt.substring(prompt.length - Math.floor(chunkSize * 0.3)); // 30% de la fin
    console.log(`Réduction drastique : combinaison du début et de la fin du contenu`);
  ...
  try {
    ...
    if (encoder.encode(trimmedPrompt).length > safeContextSize) {
      console.log(`La réduction avec splitText n'est pas suffisante, coupe brutale appliquée`);
